...
<div class="container-xxl flex-grow-1 container-p-y container-fluid">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Resume /</span> List</h4>
    <!-- Basic Bootstrap Table -->

    <div class="card">
        <div class="container-xxl d-flex flex-wrap justify-content-between py-2 flex-md-row flex-column mt-5">
            <a href="{{ route('resumeAdd') }}" class="btn btn-outline-primary btn-sm"> + ADD</a>
        </div>
            ...
            <table class="table">
                <thead>
                    <tr class="text-center">
                        ...
                        <th>Title</th>
                        <th>From-To</th>
                        <th>Sub Title / Company Naame</th>
                        <th>Status</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody class="table-border-bottom-0">
                    @forelse($resume as $key => $value)
                    <tr id="willremove{{ $value->id }}"
                        class="@if($value->status ==1 )table-success @else table-danger @endif">
                        <td><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong>{{ ucfirst($value->type)
                                }}</strong>
                        </td>
                        <td class="text-center">
                            <ul class="list-unstyled">
                                <a href="{{ asset('resumesImage') }}/{{ $value->icons }}" target="_blank">
                                    <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top"
                                        class="avatar avatar-xs pull-up" title="{{ $value->main_title }}">
                                        <img src="{{ asset('resumesImage') }}/{{ $value->icons }}" alt="Avatar"
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item text-warning"
                                        href="{{ route('resumeEdit', ['id' => $value->id]) }}"><i
